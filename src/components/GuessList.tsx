...
const completeList = (guesses: GuessLetter[][]) => {
  return [...guesses, ...Array(GUESS_LIST_SIZE - guesses.length).fill([])].map(
    (guess, index) =>
      completeArray<GuessLetter>(guess, WORD_SIZE, {
        letter: "",
        state: index === guesses.length - 1 ? "typing" : "disabled",
      })
  );
};

function GuessList(props: GuessListProps) {
  const allGuesses = completeList(props.guesses).map((guess, guessIndex) => {
    return (
      <div key={guessIndex} className="d-flex justify-content-center line">
